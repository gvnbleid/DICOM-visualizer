...
namespace DICOM_visualizer.Helpers
{
    public class GridCell
    {
        ...
        public GridCell(Slice backSlice, Slice frontSlice, int rowIndex, int columnIndex, int sliceIndex, int slicesCount, double minLoc, double maxLoc)
        {
            float xSpacing = (float)backSlice.ColumnCount / slicesCount;
            float ySpacing = (float)backSlice.RowCount / slicesCount;
            float zLength = (float)(maxLoc - minLoc);
            float zSpacing = zLength / slicesCount;
            ...
            float yBottomTransposed = yBottom - zLength / 2f;
            float yTop = (sliceIndex + 1) * zSpacing;
            float yTopTransposed = yTop - zLength / 2f;
